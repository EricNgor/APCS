...
public class Lotto extends PApplet {
    ...
    public void run() {
        ...
        /*
         * add amount gained; multiply if multiplying; multiply by 2 instead of multiplier if (powerball && tier5)
         * ↓
         */
        gainList.set(0, gain * (multiplying ? (mode == POWERBALL && gain == 1000000 ? 2 : multiplier) : 1)); 
    ...
    public void rollYourNums() {
        ...
        for (int i = 0; i < nums.length; i++) { // add 0 in front if less than 10 and convert to String array
            numsList.get(0)[i] = (nums[i] < 10 ? "0" : "") + Integer.toString(nums[i]); // Setting values of 0th String array
        }
        ...
        powList.add(0, new String(""));
        if (powNum < 10) { // convert and +0 if > 10
            powList.set(0, "0"+ Integer.toString(powNum));
        }
        else {
            powList.set(0, Integer.toString(powNum));
        }
    }
    ...
    public void dispGains(int index) {
        ...
        if (index*32 - (scrollShift*32) >= 0) {
            text("+$" + Integer.toString(gainList.get(index)), 16+(6*64), index*32 - (scrollShift*32) + 64);
        }
    }
    ...
    public void rollWinning() {
        ...
        for (int i = 0; i < nums.length; i++) { // add 0 in front if less than 10 and convert to String
            if (nums[i] < 10) {
                winningNums[i] = "0"+Integer.toString(nums[i]);
            }
            else {
                winningNums[i] = Integer.toString(nums[i]);
            }
        }
        ...
        if (powNum < 10) {
            winningPow = "0"+Integer.toString(powNum);
        }
        else {
            winningPow = Integer.toString(powNum);
        }
    ...
    public void dispWinningNums() {
        ...
        textSize(28);
        fill(255, 0, 0); 
        text(winningPow, x, y); // red powerball
        x -= 64;
        fill(255);
    ...
    public void dispSpeedAdjust() {
        ...
        else speedS = Integer.toString(speed);
    }

    public void keyPressed() {
        if (speedFocus) {
            if (key == BACKSPACE && !speedS.equals("")) speedS = speedS.substring(0, speedS.length()-1);
            else if (key == ENTER) {
    ...
    public void dispCost() {
        ...
        textSize(12);
        fill(255);
        text("Cost per ticket: $" + cost, width-16, 2*height/3-208); // y(spent)-16
    }
    ...
    public void dispSpent() {
        ...
        text("$"+Integer.toString(spent), width-16, 2*height/3-160);
    }
    ...
    public void dispWinnings() {
        ...
        text("$"+Integer.toString(winnings), width-16, 2*height/3-64);
    }
    ...
    public void dispNet() {
        ...
        text(val + Integer.toString(Math.abs(money)), width-16, 2*height/3+32);
    }
    ...
    public void dispStats() {
        ...
        for (int i = 1; i <= numTiers; i++) {
            text("TIER" + Integer.toString(i) + " win: " + tierCountList.get(i), x, y);
    ...
    public void dispPercentage(int num, int y) {
        ...
        text(Double.toString(percentage) + "%", width-16, y);
        textAlign(LEFT, CENTER);
    }
    ...
    public void mouseClicked() {
        ...
        if (mode == MENU) { // menu
            ...
            if (modeHover == COMMENCE) commencing = !commencing;
                ...
                System.out.println("final hover: " + modeHover);
                draw();
            }
